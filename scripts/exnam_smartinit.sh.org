...
#                 00        ak,conus,hi,pr,aknest3,conusnest2p5
#                 03-09     ak  (to create ak_rtmages files)
#                 03-54/60  alaskanest, conus,hi,pr, aknest3,conusnest2p5
#                 >ENDHR-84 ak,conus,hi,pr
...
# f66 aren't split between nest and parent
  ...
  case $RUNTYP in
                      ak) export ENDHR=12;;        # ak_rtmages files
                      gm) export ENDHR=48;;       
...
#   Downscale from NAM parent for forecast hrs=0 and > ENDHR (54/60)
  ...
  if [ $ffhr -gt 0 -a $ffhr -le $ENDHR ];then
    ...
    case $RUNTYP in
      alaskanest) export RUNTYP=ak;;
    esac
  fi

  ${NAM_SMINIT_SSH:-$USHnam/nam_smartinit.sh}
