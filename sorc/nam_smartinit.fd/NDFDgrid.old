      subroutine NDFDgrid(psfc,zsfc,t,z,q,u,v,p,t2,q2,d2,
     x      u10,v10,veg_nam_ndfd,tnew,dewnew,unew,vnew,
     x      qnew,topo_ndfd)

      PARAMETER(IM=1073,JM=689,LM=60)
     ...
     2      QNEW(IM,JM)
      real VEG_NDFD(IM,JM),VEG_NAM_NDFD(IM,JM),TOPO_NDFD(IM,JM)
      real TTMP(IM,JM),DTMP(IM,JM),UTMP(IM,JM),VTMP(IM,JM)
      real exn0,exn1, wsp
      integer change(im,jm)
...
c  read in 5 km topography
C  change name for consistency with non-conus region names
C  CHANGE FOR GRIB FILES for non-conus regions ?????
       open (46, file='TOPONDFD',
     x      form='unformatted')
       read (46) topo_ndfd
       close (46)

c  read in 5 km vegetation
c change to unit 48 for consistency with other domains
       open (48, file='LANDNDFD',
     x        form='unformatted')
       read (48) veg_ndfd
       close (48)
       ...
       do j=1,jm
       do i=1,im
         if (veg_nam_ndfd(i,j).eq.16.)
     1       veg_nam_ndfd(i,j) = -1.

         if (veg_nam_ndfd(i,j).ne.16..and.
     1      veg_nam_ndfd(i,j).gt.0.)
     1       veg_nam_ndfd(i,j) = 0.10

         if (veg_nam_ndfd(i,j).eq.-1.)
     1       veg_nam_ndfd(i,j) =  0.
       end do
       end do
...
C --- q = specific humidity at 2m from NAM model sfc
c          qq = q2(i,j)
c          qv = qq/(1.-qq)
c          qv2m = qv
...
C -- again, use 2m T at NAM regular terrain from similarity
C      theory for derivation of 2m T at topomini elevation
          tsfc = t2(i,j) + (zs-topo_ndfd(i,j))*gam

c  Don't let reduced valley temps be
c     any lower than NAM 2m temp minus 10K.
          tsfc = max(t2(i,j)-10.,tsfc)
c  Can't let valley temps go below NAM dewpoint temps.
...
C============================================
C ----       Now only if topo_NDFD is above the NAM model elevation

c         Here, when topo-NDFD > topo-NAM, we allow a small
...
C --- temperature
c          tup = thetavc*(pnew(i,j)/P1000)**rovcp_p
c     1       / (1.+0.6078*qc)
c          tup=thetavc
c GSM changed the tup computation, as it appears to give
c   a better-looking product.  need to revisit at some point 
          tup=t2(i,j)+frac*(t(i,j,k)-t2(i,j))
            
c  provisional 2m temp at NDFD topo
          tnew(i,j) = t2(i,j) + (tup-t1)

C --- Don't let extrapolated temp to be any larger than
C       the value at the NAM terrain level.
C     This will avoid the problem with NDFD temp values
C       being set to be much warmer than NAM 2m temp.
...
C ---   Just use q at NAM 1st level in this case.
c     should use q2, but the values don't look good
C       Obtain Td corresponding to NDFD pres otherwise.
          qv=q(i,j,1)
c           qv=q2(i,j)
...
C============================================
C -- use land mask to get better temps/dewpoint/winds
C      near coastlines.
C    Use nearest neighbor adjustment where NAM
...
c   values don't get shifted around in the adjustment
       ...
       do i=1,im
         ...
         utmp(i,j)=unew(i,j)
         vtmp(i,j)=vnew(i,j)
         rough_mod(i,j) = veg_nam_ndfd(i,j)
       end do
       end do
...
c          write (6,*)'Changed water to land',i,j
       ...
       do j=1,jm
       do i=1,im
         if (veg_nam_ndfd(i,j).gt.0.05 .and.
     1      rough_mod(i,j).lt.0.05) then
C -----------------------------------------------------------------
C -- i.e.  NDFD grid-point is over WATER (per rough_mod)
C          NAM-interp grid-point is over LAND
C -----------------------------------------------------------------
...
884       continue
          ...
          if (veg_nam_ndfd(i,j).lt.0.05 .and.
     1      rough_mod(i,j).gt.0.05) then
C -----------------------------------------------------------------
C -- i.e.  NDFD grid-point is over LAND (per rough_mod)
C          NAM-interp grid-point is over WATER
C -----------------------------------------------------------------
