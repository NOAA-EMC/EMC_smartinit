...
!   13-07-15  J.McQueen - Modified to read pressure level fields over GM
!   (non nam, hiresw model)
...
!                  ENVVAR not needed
!                  Introduced RDHDRS and SETVAR routines to eliminate
!                  redundancies
...
!     LUFB=31; LUFI=32   !DEFAULT MODEL FIXED FIELD GRIB FILE UNITS
      if (.not.lanl) then
       LUGP3=11;  LUGP3i=12
       LUGS3=11;  LUGS3i=12
       LUGP6=11;  LUGP6i=12
...
!     FOR 12-hr on-cycle TIMES, WE NEED 3 AND 6-HR BUCKETS AND MAX/MIN
!     TEMP
...
!     if(mdl .eq. 'NARR')then
!       print*, 'Reading fixed fields'
!       CALL RDHDRS(LUGB,LUGI,IGDNUMF,GDIN,NUMVALF)
!     endif
...
!      2) temporarily fill the 12th time slot, since we have only 11
!      here
      THOLD=0.
      DHOLD=0.
...
!     if(mdl .eq. 'NARR')then
!       J=0;JPDS=-1;JGDS=-1;JPDS(3) = IGDNUMF
!       JPDS(5) = 007
!       JPDS(6) = 001
!       CALL SETVAR(LUFB,LUFI,NUMVALF,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,ZSFC,IRET,ISTAT)
!       print*,'Reading sfc height'
!       print*,'max/min sfc height',maxval(zsfc),minval(zsfc)
!     else
...
! get surface pressure
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,PSFC,IRET,ISTAT)
        print*,'Reading sfc pres'
        print*,'max/min sfc pres',maxval(psfc),minval(psfc)
...
! Moved to hourly reads for hourly writes for RTMA (03-19-2013)
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,VIS,IRET,ISTAT)
        print*,'Reading visibility'
        print*,'max/min visibility',maxval(vis),minval(vis)

! Planetary boundary layer height (m) HPBL
      JPDS=-1;J=0
      JPDS(5) = 221
      JPDS(6) = 001
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,HPBL,IRET,ISTAT)
        print*,'Reading pbl height'
        print*,'max/min pbl height',maxval(hpbl),minval(hpbl)
...
! 2-m temp
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,T2,IRET,ISTAT)
        print*,'max/min 2m temp',maxval(t2),minval(t2)
...
! 2-m spec hum
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,Q2,IRET,ISTAT)
        print*,'max/min 2m q',maxval(q2),minval(q2)
...
! 2-m dew point
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,D2,IRET,ISTAT)
        print*,'max/min 2m td',maxval(d2),minval(d2)
...
! 10-m U
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,U10,IRET,ISTAT)
        print*,'max/min 10m u',maxval(u10),minval(u10)
...
! 10-m V
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,V10,IRET,ISTAT)
        print*,'max/min 10m v',maxval(v10),minval(v10)
...
!       if(mdl .eq. 'NARR')then
!         CALL SETVAR(LUFB,LUFI,NUMVALF,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,VEG,IRET,ISTAT)
!       print*,'max/min veg',maxval(veg),minval(veg)
!       else
...
! lowest wet bulb zero level
      print*,'skipping lowest web bulb zero'
      if (REGION .ne. 'GM' ) then
      if (mdl .ne. 'NARR') then
      JPDS=-1;J=0;JPDS(3) = IGDNUM
      JPDS(5) = 7
...
! Best Liftex Index
      ...
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,BLI,IRET,ISTAT)
        print*,'max/min bli',maxval(bli),minval(bli)
      endif

!====================================================================
!      READ PRECIP FROM FULL (LUGB=11) or SPECIAL GRIB FILE (unit 15-21)
!  STANDARD FULL GRIB FILE PRECIP
!      For on-Cycles it will have either a 3-hr, 6-hr, 9-hr, or 12-hr
!      accumulation
...
!     KLTYP=109   !Hybrid vertical levels
      KLTYP=100   !Pressure levels
        IF (REGION.EQ.'GM' .or. mdl .eq. 'NARR') THEN
          KLTYP=100  !Pressure level file
          do ii=1,IMAX
          do jj=1,JMAX
          PMID(ii,jj,1)=1000.;PMID(ii,jj,2)=975.;PMID(ii,jj,3)=950.;PMID(ii,jj,4)=925.;PMID(ii,jj,5)=900.
          PMID(ii,jj,6)=875.;PMID(ii,jj,7)=850.;PMID(ii,jj,8)=825.;PMID(ii,jj,9)=800.;PMID(ii,jj,10)=775.
          PMID(ii,jj,11)=750.;PMID(ii,jj,12)=725.;PMID(ii,jj,13)=700.;PMID(ii,jj,14)=650.;PMID(ii,jj,15)=600.
          PMID(ii,jj,16)=550.;PMID(ii,jj,17)=500.;PMID(ii,jj,18)=450.;PMID(ii,jj,19)=400.;PMID(ii,jj,20)=350.
          PMID(ii,jj,21)=300.;PMID(ii,jj,22)=275.;PMID(ii,jj,23)=250.;PMID(ii,jj,24)=225.;PMID(ii,jj,25)=200.
          PMID(ii,jj,26)=175.;PMID(ii,jj,27)=150.;PMID(ii,jj,28)=125.;PMID(ii,jj,29)=100.
...
!   get the vertical profile of height
        ...
        print*,'ll,pres,max/min hght',ll,pmid(1,1,ll),maxval(hght(:,:,ll)),minval(hght(:,:,ll))
...
!   get the vertical profile of temperature
        ...
        print*,'ll,pres,max/min temp',ll,pmid(1,1,ll),maxval(t(:,:,ll)),minval(t(:,:,ll))
...
! JTM 01-28-13: Added check for where previous temps are not at validpts
         do ii=1,imax
         do jj=1,jmax
           if(t(ii,jj,1).le.10)print*,'bitmap point, i,j=',ii,jj
...
!   get the vertical profile of q
       JPDS(3)=IGDNUM; JPDS(5)=051; JPDS(6)=KLTYP
       CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,Q(:,:,LL),IRET,ISTAT)
        print*,'ll,pres,max/min Q',ll,pmid(1,1,ll),maxval(q(:,:,ll)),minval(q(:,:,ll))
...
!   get the vertical profile of u
        ...
        print*,'ll,pres,max/min uwnd',ll,pmid(1,1,ll),maxval(uwnd(:,:,ll)),minval(uwnd(:,:,ll))
...
!   get the vertical profile of v
        ...
        print*,'ll,pres,max/min vwnd',ll,pmid(1,1,ll),maxval(vwnd(:,:,ll)),minval(vwnd(:,:,ll))
...
!   get the vertical profile of cloud fraction for non-nests
      if (.not. lnest) then
       ...
       DO LL=1,KMAX
        ...
        print*,'ll,pres,max/min cld frac',ll,pmid(1,1,ll),maxval(cfr),minval(cfr)
        J=K
       ENDDO
      endif

! Convert from mb to hPa
          PMID=PMID*100.
...
!   950 mb temperature
      ...
      if (REGION.NE.'GM')then
        ...
        print*,'ll,pres,max/min t950',ll,pmid(1,1,ll),maxval(t950),minval(t950)
      endif

!   850 mb temperature
      JPDS(7) = 850
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,T850,IRET,ISTAT)
        print*,'ll,pres,max/min t850',ll,pmid(1,1,ll),maxval(t850),minval(t850)

!   700 mb temperature
      JPDS(7) = 700
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,T700,IRET,ISTAT)
        print*,'ll,pres,max/min t700',ll,pmid(1,1,ll),maxval(t700),minval(t700)

!   500 mb temperature
      JPDS(7) = 500
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,T500,IRET,ISTAT)
        print*,'ll,pres,max/min t500',ll,pmid(1,1,ll),maxval(t500),minval(t500)

!     nests already have computed cld fracs...
!     need to move so cld fracs are used at the analysis time [AMG]
      if (lnest) then
        J=0;JPDS=-1
        JPDS(3)=IGDNUM
        JPDS(5) = 71
        ...
        print*,'Reading Total cloud cover'
        print*,'max/min tcld',maxval(tcld),minval(tcld)
        JPDS(5) = 73
        JPDS(6) = 214
        CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,LCLD,IRET,ISTAT)
        print*,'Reading low cloud cover'
        print*,'max/min lcld',maxval(lcld),minval(lcld)
        JPDS(5) = 74
        JPDS(6) = 224
        CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,MCLD,IRET,ISTAT)
        print*,'Reading middle cloud cover'
        print*,'max/min mcld',maxval(mcld),minval(mcld)
        JPDS(5) = 75
        JPDS(6) = 234
        CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF, K,KPDS,KGDS,MASK,GRID,HCLD,IRET,ISTAT)
        print*,'Reading high cloud cover'
        print*,'max/min hcld',maxval(hcld),minval(hcld)
       endif

! 30 mb boundary layer averages - Temperature
      if(mdl .eq. 'NARR')then
      J=0
      JPDS(3) = IGDNUM
      JPDS(5) = 011
      JPDS(6) = 116
      JPDS(7) = 7680
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T30,IRET,ISTAT)
        print*,'Reading Temp at 0-30 mb'
        print*,'max/min T30',maxval(t30),minval(t30)

      JPDS(7) = 15390
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T60,IRET,ISTAT)
        print*,'Reading Temp at 60-30 mb'
        print*,'max/min T60',maxval(t60),minval(t60)

      JPDS(7) = 23100
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T90,IRET,ISTAT)
        print*,'Reading Temp at 90-60 mb'
        print*,'max/min T90',maxval(t90),minval(t90)

      JPDS(7) = 30810
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T120,IRET,ISTAT)
        print*,'Reading Temp at 120-90 mb'
        print*,'max/min T120',maxval(t120),minval(t120)

      JPDS(7) = 38520
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T150,IRET,ISTAT)
        print*,'Reading Temp at 150-120 mb'
        print*,'max/min T150',maxval(t150),minval(t150)

      JPDS(7) = 46230
      CALL SETVAR(LUGB,LUGI,NUMVAL,J,JPDS,JGDS,KF,K,KPDS,KGDS,MASK,GRID,T180,IRET,ISTAT)
        print*,'Reading Temp at 180-150 mb'
        print*,'max/min T180',maxval(t180),minval(t180)
