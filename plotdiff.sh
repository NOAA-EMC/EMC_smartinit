...
#             CONTROL(d2)  vs EXP
set -A dirin ${wkroot}/d2${mdld}${drt[0]}${cyc}  ${wkroot}/${mdld}${drt[0]}${cyc}
...
izoom=1
case $plotarea in pr|pu)izoom1=1;izoom2=1;;esac
...
stnplt=${stnplt:-"32/${stsz}//${stwd}/0|2|sfstns.tbl"}
if [ $izoom3 -gt 0 ];then stnplt="32/${stsz}//${stwd}/0|2|sfstns.tbl";fi

case ${plotarea} in
                  dset|ak|ak3 ) stnplt=;skipv[plt]="/20;20";;
  conus|conus2p5|conusnest|12 ) stnplt=;skipv[plt]="/25;25";;
                            * )
#   mapf="HIRVUS.USG + HISTUS.NWS "
#TEST    mapf="LORVWO.CIA + HISTUS.NWS "
#TEST    mapf="histus.nws+HIPOWO.CIA"
...
case $vhdr in 
 TMPF|DWPF|TMPC|TMXK|TMNK|VSBY )
   ovly=r
   case $mdlin in  nam|urma2p5) glevel=2;;esac  # if 1st model is not smart, then change glevel from 0 to 2
 ...
 SPED|GUST|SKNT )
   ...
   case $mdlin in  nam|urma2p5)
     if [  $vhdr = SPED ];then glevel=10;fi;;
   esac
   fint="-10;-8;-6;-4;-2;-1;1;2;4;6;8;10"
   FLINE="28-23;31;20-12";;
 DRCT )
...
if [ "$ovly" = "r" -a -s "$obsfile" ];then
  ...
  sfmap >> sfmap${vhdr}.log << EOF
   ...
   RADFIL   =
   IMCBAR   =
   SFPARM   = brbk:1.2:3;sknt:1.0:2
   DATTIM   = $dattim
   SFFILE   = ${obsfile}
   COLORS   = 32;4; 3; 29;22; 15;  18;  6;   1
   MAP      = 0
   MSCALE   = 
   TITLE    = 0
   CLEAR    = NO
