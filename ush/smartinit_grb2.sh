...
# 2015-04     Annette Gibbs : Added GRIB2 option
#======================================================================
...
if [ $GRIBopt = 2 ];then
case $RUNTYP in
# conusnest2p5) mdlgrd=conusnest; rg=con; outreg=conus2p5; wgrib2def="lambert:265:25:25 238.446:2145:2540 20.192:1377:2540";;
  ...
  hi) rg=hi; mdlgrd=hawaiinest; compress="jpeg";outreg=hi; wgrib2def="mercator:20 198.475:321:2500:206.131 18.073:225:2500:23.088";;
  pr) rg=pr; mdlgrd=priconest; compress="jpeg"; outreg=pr; wgrib2def="mercator:20 291.804:177:2500:296.028 16.829:129:2500:19.747";;
# pr1p5) mdlgrd=priconest; rg=pr; compress="jpeg"; outreg=pr1p5; wgrib2def="mercator:20 291.972167:339:1500:296.0156 16.977485:225:1500:19.52200";;
...
if [ $GRIBopt = 1 ];then
  $utilsorc/copygb -g "$grid" -x srefallpcp srefpcp${rg}_${SREF_PDY}${srefcyc}f0${pcphrl}
...
####set -x
for fhr in $hours; do
#  if [ $fhr -eq 2 -o $fhr -eq 02 ];then exit;fi
  rm -f *out${fhr}
...
#else
#      cp ${mdlin}${FHRFRQ}${text}.grib2 WRFPRS${FHRFRQ}.tm00
#fi
...
#else
#        cp ${mdlin}${fhr6}${text}.grib2 WRFPRS${fhr6}.tm00
#fi
        case $natgrd in bgrd3d) 
...
if [ $GRIBopt = 1 ];then 
      $utilsorc/copygb -g "$cpgbgrd" -i3 -x ${freq}precip.${fhr} ${freq}precip
...
#    $copygbexec/copygb -g "$cpgbgrd"  WRFPRS${fhr}.tm00 WRFPRS${fhr}i.tm00 ${prdgfl}
#  else
#POINT TO NETwork prdgen (/nwprod/exec) 
#   ${EXECmdl}/${mdl}_prdgen < input${fhr}.prd > prdgen.out${fhr}
    /scratch2/portfolios/NCEPDEV/meso/save/Eric.Rogers/nwprod_BE/sorc/nam_prdgen.fd/nam_prdgen < input${fhr}.prd > prdgen.out${fhr}
    export err=$?;  err_chk
# fi
#188  fi
fi # GRIBopt fi
...
#     For 3 hourly input files, hourly maxmins not created
    ...
    if [ -s MAXMING2.f${fhr1} ];then
      echo MAXMING2.f${fhr1} FOUND
      cp MAXMING2.f${fhr2} MAXMIN2
      cp MAXMING2.f${fhr1} MAXMIN1
    else
#     For 3 hourly input files, hourly maxmins not created
      echo MAXMIN$fhr1.tm00 NOT FOUND....3 hrly imputs assumed
      ln -fs meso${rg}.NDFDf${fhr} MAXMIN2
...
# Save hourly ak,hi,pr,conus2p5 nests and ak_rtmages(from nam parent) for RTMA 1st guess fields
  if [ $fhr -le $hrlyfhr ];then
    if [ $GRIBopt = 2 ];then
      cp smartg2.${fhr} MESO${RGIN}${fhr}.tm00
    fi
    if [ $GRIBopt = 1 ];then
    case $RUNTYP in
     ak_rtmages) 
    ...
    case $RUNTYP in
     ak_rtmages) 
       cp MESO${RGIN}${fhr}.tm00  $COMOUT/${mdl}.t${cyc}z.smart${RUNTYP}${fhr}.tm00.grib2
       mksmart=0;;
     hawaiinest|priconest|conusnest2p5|aknest3)
       cp MESO${RGIN}${fhr}.tm00  $COMOUT/${mdl}.t${cyc}z.smart${outreg}${fhr}.tm00.grib2
       if [ $RUNTYP = conusnest2p5 ];then mksmart=1;fi;;  #make grib2 files for wave group
   esac
  fi
fi
