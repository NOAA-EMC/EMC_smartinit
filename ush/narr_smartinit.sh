...
if [ $mdl = narr ];then hours="${ffhr}";fi   #DGEX only has output every 3 hrs
...
#         Check that 00 hr analysis is from NDAS or GDAS (08/2013)
          if [ $fhr -eq 00 -a $GUESS = GDAS ];then
            ...
            echo; echo "NARR data"
##            mdlin=$COMIN/EGDAWP00.20000727
            mdlin=$COMIN/merged_AWIP32.19920930
            cp ${mdlin}${fhr} WRFPRS${fhr}.tm00
##            cp /meso2/noscrub/Annette.Gibbs/NARR/fixed_files/EGDFIX*.*.fixed* .
##            cat WRFPRS${fhr}.tm00 EGDFIX.topo.fixed > WRFPRS${fhr}.tm00_withfixed
            cp /meso2/noscrub/Annette.Gibbs/NARR/fixed_files/AWIP32*.*.fixed* .
            cat WRFPRS${fhr}.tm00 AWIP32.topo.fixed > WRFPRS${fhr}.tm00_withfixed
            mv WRFPRS${fhr}.tm00_withfixed WRFPRS${fhr}.tm00
...
# Test copygb option instead of prdgen for NARR data 
# Using i=0 bi-linear interpolation
# if [ $RUNTYP = conusnest2p5 ];then
    cp /meso2/noscrub/Annette.Gibbs/NARR/namelist .
    $utilexec/copygb -N namelist -g "$cpgbgrd"  WRFPRS${fhr}.tm00 WRFPRS${fhr}i.tm00 ${prdgfl}
# Vegtyp should have nearest neighbor interpolation (-i2)
##    $utilexec/copygb -N namelist -g "$cpgbgrd"  -i2 EGDFIX.vgtyp.fixed EGDFIXi.vgtyp.fixed vgtyp.grb
    $utilexec/copygb -N namelist -g "$cpgbgrd"  -i2 AWIP32.vgtyp.fixed AWIP32i.vgtyp.fixed vgtyp.grb
    cat ${prdgfl} vgtyp.grb > ${prdgfl}.grb
    mv ${prdgfl}.grb ${prdgfl}
#   $utilexec/copygb -g "$cpgbgrd"  EGDFIX.fixed EGDFIXi.fixed mesocon.fixed
# fi
