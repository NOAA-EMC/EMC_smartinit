...
!--------------------------------------------------------------------------------------
   ...
   SUBROUTINE SNOWFALL(SN0,SNOWAMT,DOWNT,THOLD,GDIN,AVG)
        use grddef
        TYPE (GINFO), INTENT(IN) :: GDIN
   ...
   SUBROUTINE MKPOP(PBLMARK,RH,BLI,PCP01,PCP10,PXCP01,PXCP10,QPF,POP,GDIN,IAHR)
        use grddef
        TYPE (GINFO), INTENT(IN) :: GDIN
...
!-----------------------------------------------------------------------------------------
      ...
      IF(.NOT.LCYCON.AND.MOD(FHR+6,12).EQ.0) LHR12=.TRUE.
...
!==========================================================
    ...
    IM=GDIN%IMAX;JM=GDIN%JMAX;KMAX=GDIN%KMAX;ITOT=NUMVAL
...
!  CALL THE DOWNSCALING CODE 
       ...
       CALL NDFDgrid(VEG,DOWNT,DOWNDEW,DOWNU,DOWNV,DOWNQ,TOPO,VEG_NDFD,gdin)
...
! 3-hr POP
        CALL MKPOP(PBLMARK,RH,BLI,P3CP01,P3CP10,P12CP01,P12CP10,QPF3,POP3,GDIN,3)
...
! 6-hr POP
       IF(MOD(FHR,6).EQ.0) THEN
         CALL MKPOP(PBLMARK,RH,BLI,P6CP01,P6CP10,P12CP01,P12CP10,QPF6,POP6,GDIN,6)
...
! 12-hr POP
       IF (LHR12) THEN
        IF(LCYCON .OR. .NOT.LCYCON.AND.FHR.NE.6) THEN
         CALL MKPOP(PBLMARK,RH,BLI,P12CP01,P12CP10,P12CP01,P12CP10,QPF12,POP12,GDIN,12)
      ...
      CALL MAKESTRING(IRAIN,ISNOW,IZR,IIP,BLI,POP3,GDIN,WXSTRING,GRIDWX)
...
!JTM     Corrected  ERROR...Added *100 to compute rhavg 11/25/12
        ...
        ENDDO
        ENDDO
!????   HI nest uses 25 for max limit ???       
        WHERE (QPF3.GT. 0.10) CWR=AMAX1(CWR,30.)
...
! HI snowamt3 uses temp < 264 check ????
        CALL SNOWFALL(SN03,SNOWAMT3,DOWNT,THOLD,GDIN,3.)
        ID(8)=66;ID(9)=1
        ID(18)=FHR3;ID(19)=FHR
        ID(20)=4
        DEC=3.0
        ...
        IF (MOD(FHR,6).EQ.0)  THEN  
          CALL SNOWFALL(SN06,SNOWAMT6,DOWNT,THOLD,GDIN,4.)
          ID(18)=FHR6;ID(19)=FHR
...
!JTM        call smoothpm(sky,2)  ! AWAITING LIBRARY
...
!--------------------------------------------------------------------------
      ...
      DO J=1,JM
       ...
       ENDDO
      ENDDO
      WHERE(DIRTRANS.LT.0.)   DIRTRANS=DIRTRANS+360.0
      WHERE(DIRTRANS.GT.360.) DIRTRANS=DIRTRANS-360.0    
      ...
      DO I=1,IM
       ...
       DO L=2,KMAX
         ...
         IF (theta.gt.firetheta) THEN
           ...
           EXIT   !JTM Test rather than GOTO 
         ENDIF
       ENDDO
      ENDDO
      ENDDO
...
!----------------Make into subroutine lal
    ...
    ENDIF  ! 3 hour writes
      ...
      IF (LCYCON .AND. MOD(FHR,12) .NE.0 .OR. .NOT.LCYCON .AND. MOD(FHR-6,12).NE.0) THEN
...
!   since this files serve as 1st guess for Alaskan RTMA
      IF (TRIM(REGION).EQ.'HI')THEN
        IF(MOD(FHR,3).NE.0 .AND. FHR.LT.9) CALL GRIBLIMITED(70,GDIN)
      ENDIF

!  write older T/Td data for max/min to grib file
      ALLOCATE (TEMP1(IM,JM),TEMP2(IM,JM),STAT=kret)
      IF (FHR.NE.0. .AND. MOD(FHR,3).EQ.0) THEN
        IF (.NOT.LHR12) THEN
          DO ivarb=1,2
            ID(1:25) = 0
            DEC=-2.0
            IF(ivarb.eq.1) then
              TEMP1=THOLD(:,:,2);TEMP2=THOLD(:,:,3)
              ID(8)=11;ID(9)=1
            else
              TEMP1=DHOLD(:,:,2);TEMP2=DHOLD(:,:,3)
              ID(8)=17;ID(9)=1
            endif
...
!       calcmax(psfc,thold,dhold,downt,downdew,tmax,tmin,rhmax,rhmin)kj
        TMAX12=-9999.;TMIN12=9999.
        RHMAX12=-9999.;RHMIN12=9999.
...
!          print *,I,J,L,LHR12,FHR,THOLD(I,J,L),TMIN12(I,J)
         ENDDO
        ENDDO
        ENDDO
...
!---------------------------------------------------------------------------
        ...
        ALLOCATE (TEMP1(IM,JM),TEMP2(IM,JM),STAT=kret)
         ...
         WHERE (SN0.GT.0.) 
           ...
           WHERE (TEMP1.LT.264..AND.avg.gt.3) 
             TEMP2=20.
           ELSEWHERE
             TEMP2=(273.15-TEMP1)/idiv+8.   !SNOWR
           ENDWHERE
         END WHERE
...
!--------------------------------------------------------------------------------
      ...
      IF (IAHR.EQ.3 .AND. IFHR .GT. 11) THEN
        ...
        WHERE (PCP01 .GT. PXCP01)
          TMPPCP=(PCP01+PXCP01)/2.
          PCP01  = TMPPCP
          PXCP01 = TMPPCP
        END WHERE

        WHERE (PCP10 .GT. PXCP10)
          TMPPCP=(PCP01+PXCP01)/2.
          PCP10  = TMPPCP
          PXCP10 = TMPPCP
        END WHERE
...
!--------------------------------------------------------------------------------
        ...
        DENOM=3.
        BL=BLI(I,J) 
        IF (IAHR.EQ.6) THEN;POPFACT=2;DENOM=5;BL=0.;ENDIF
...
!    so important here, opted to give more weight to the SREF
        ...
        ELSE
          ...
          IF (POPTMP .GT. 70.) & 
            POP(I,J)=(POPFACT*POPTMP+POPFACT*PCP01(I,J)+PCP10(I,J))/DENOM
        
          IF (IAHR.GE.6) THEN 
            IF(POPTMP.GE.30 .AND. POPTMP.LE.70) POP(I,J)=AMAX1((POPTMP+PCP01(I,J))/2.,20.)
            IF(IAHR.EQ.6 .AND. POPTMP .LT. 30.) POP(I,J)=(POPTMP+2*PCP01(I,J)+PCP10(I,J))/4.
          ENDIF
        ENDIF
...
!---------------------------------------------------------
    ...
    INCLUDE 'DEFGRIBINT.INC'   ! interface statements for gribit subroutines
       ...
       print *,'OUTPUT LIMITED GRIB FILE at FHR ',FHR,' for REGION ',REGION
       RITEHD = .TRUE.
       ID(1:25) = 0
       ID(8)=11
       ID(9)=1
