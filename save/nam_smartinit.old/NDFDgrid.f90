...
!  read in 5 km vegetation for CONUS domain
       ...
       close (48)
        ...
        J=0;JPDS=-1;JPDS(3)=IGDNUM;JPDS(5)=81;JPDS(6)=1
...
!---> Alaska, Choose q at 1st level for more realistic output 
      if (gdin%region .eq. 'AK') qv=q(i,j,1)
...
! ----------------------------------------------------
       ...
       do i=1,im
        ...
        ip1 = min(im,i+1)
        ladjland=.false.
        if (gdin%region .eq. 'CS' .and. veg_ndfd(i,j).eq.veglim) ladjland=.true.
        if (gdin%region .ne. 'CS' .and. veg_ndfd(i,j).lt.veglim) ladjland=.true.
        if (ladjland) then
