...
CJTM  IF (FHR .EQ. 0) THEN
CJTM   print *, 'READ GRIB ANALYSIS FHR=', $FHR
CJTM   CALL GETGRIB_ANL(PSFC,ZSFC,PMID,HGHT,T,Q,UWND,VWND,
CJTM X    CFR,T2,Q2,D2,U10,V10,VEG,BLI,WETFRZ,VIS,T950,T850,T700,
CJTM X    T500,RH850,RH700,GUST,REFC,DATE,FHR)
CJTM   GOTO 222 
CJTM  ENDIF

CJTM  IF (CYC .EQ. 00 .OR. CYC .EQ. 12) THEN
CJTM   IF (MOD(FHR,3).EQ.0) THEN 
        print *, 'READ GRIB FCST on CYC', CYC, 'FHR=', FHR

CJTM    GETGRIB: Combined off and on-cycle runs (removed GETGRIB_OFF)
CJTM             Combined for limited off-hour reads (removed
CJTM                                                  GETGRIB_LIMITED)
CJTM             Combined for fhr=0 analysis run (removed GETGRIB_ANL)
     ...
     X    P12CP10,P12CP50,THOLD,DHOLD,DATE,FHR,CYC)

CJTM X    P12CP10,P12CP50,THOLD,DHOLD,DATE,FHR)
CJTM   ELSE
CJTM    print *,'READ LMTED GRIB FCST on CYC', CYC, 'FHR=',FHR
CJTM    CALL GETGRIB_LIMITED(PSFC,ZSFC,PMID,HGHT,T,Q,UWND,VWND,T2,
CJTM X     Q2,D2,U10,V10,VEG,DATE)
CJTM   ENDIF
CJTM  ELSE
CJTM   IF (MOD(FHR,3).EQ.0) THEN
CJTM    print *,'READ GRIB FCST OFF CYC', CYC, 'FHR=',FHR
CJTM    CALL GETGRIB_OFF(PSFC,ZSFC,PMID,HGHT,T,Q,UWND,VWND,CFR,
CJTM X    ISNOW,IZR,IIP,IRAIN,T2,Q2,D2,U10,V10,VEG,BLI,WETFRZ,
CJTM X    VIS,T950,T850,T700,T500,RH850,RH700,GUST,REFC,P03M,P06M,
CJTM X    P12M,SN03,SN06,P3CP01,P3CP10,P3CP50,P6CP01,P6CP10,
CJTM X    P6CP50,P12CP01,P12CP10,P12CP50,THOLD,
CJTM X    DHOLD,DATE,FHR)
CJTM   ELSE
CJTM    print *,'READ LMTED GRIB FCST OFF CYC', CYC, 'FHR=',FHR
CJTM    CALL GETGRIB_LIMITED(PSFC,ZSFC,PMID,HGHT,T,Q,UWND,VWND,T2,
CJTM X     Q2,D2,U10,V10,VEG,DATE)
CJTM   ENDIF
CJTM  ENDIF  
...
c       use higher SREF thresholds for convective regimes.
         ...
         ENDIF
C        if (POP3(i,j).GE.50.)
C    +   print *, I,J,'POP3',POPTMP3,POP3(I,J),P3CP01(I,J),P3CP10(I,J)        
        ENDDO
        ENDDO
